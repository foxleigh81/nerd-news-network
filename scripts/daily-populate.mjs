...
function splitSentences(text) { return stripHtml(text).replace(/\s+/g,' ').split(/(?<=[.!?])\s+/).map(s => s.trim()).filter(s => s.length > 35 && s.length < 260); }
...
function parseFeed(xml, source) { const items = [];
  ...
  for (const m of xml.matchAll(/<entry\b[^>]*>([\s\S]*?)<\/entry>/gi)) { const b=m[1]; const linkTag = (b.match(/<link\b[^>]*rel=["']alternate["'][^>]*>/i)||b.match(/<link\b[^>]*>/i)||[''])[0]; items.push({ title: stripHtml(textBetween(b,'title')), link: attr(linkTag,'href') || textBetween(b,'link') || textBetween(b,'id'), published: textBetween(b,'published') || textBetween(b,'updated'), summary: stripHtml(textBetween(b,'summary') || textBetween(b,'content')) }); }
  return items;
}
...
for (const cat of categories) {
  ...
  for (const source of sources.filter(s=>s.category_id===cat.id)) {
    try {
      ...
      for (const item of parseFeed(xml, source)) {
        ...
        candidates.push({ source, title:item.title, url, published, summary:item.summary, score: recency + source.weight + topic });
      }
  ...
  for (const cand of candidates) {
    ...
    try {
      ...
      const img = extractOgImage(html, cand.url); if (!img?.url) { diagnostics.push(`${cat.slug}/${cand.title}: rejected no hero image`); continue; }
      const pageText = extractArticleText(html);
      const title = cand.title && cand.title.length > 8 ? cand.title : (html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1] ? stripHtml(html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)[1]) : null);
      if (!title) continue;
      ...
      const row = { slug, headline: title.slice(0,180), blurb: makeBlurb(title,cand.summary,pageText), body: makeBody({title,sourceName:cand.source.name,summary:cand.summary,pageText}), hero_image: img.url, hero_image_alt: (img.alt || `Lead image for “${title}”.`).slice(0,280), hero_credit: `Image: ${cand.source.name}`, thumbnail_image: img.url, thumbnail_alt: (img.alt || `Thumbnail image for “${title}”.`).slice(0,280), category_id: cat.id, author:'NNN Staff', source_name:cand.source.name, source_url:cand.url, published_at:cand.published };
      ...
      const info = db.prepare(`INSERT INTO articles (slug, headline, blurb, body, hero_image, hero_image_alt, hero_credit, thumbnail_image, thumbnail_alt, category_id, author, source_name, source_url, published_at) VALUES (@slug,@headline,@blurb,@body,@hero_image,@hero_image_alt,@hero_credit,@thumbnail_image,@thumbnail_alt,@category_id,@author,@source_name,@source_url,@published_at)`).run(row);
      inserted.push({ id: info.lastInsertRowid, category: cat.slug, source: cand.source.name, title }); existingUrls.add(cand.url); perSourcePicked.set(cand.source.id, used+1); needed--;
